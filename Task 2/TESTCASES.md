...
---
## Комментарий
В документации в Postman в разделе **Not Found** показан пример объекта (пример ответа приведен в **Ожидаемом результате** этого документа). Коды состояния совпадают, однако тела ответов не совпадают, то есть несоответствие между документацией и фактическим поведением (Приоритет Low).
...
**Response_2:** Сервис некорректно обрабатывает спецсимвол ! в ID — в сообщении об ошибке строка искажается и отображается в виде закодированных символов. Вероятно, проблема в механизме экранирования.
